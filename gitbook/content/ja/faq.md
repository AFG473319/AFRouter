...
**AFRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、AFRouter自体は100%無料でオープンソースです。**
...
**AFRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## AFRouterをセルフホストできますか?

**はい! AFRouterは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.afrouter`)
...
**はい、AFRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.afrouter`(またはカスタム`DATA_DIR`)にローカル保存
- AFRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
