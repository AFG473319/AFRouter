...
const nowFails = r.testResults.flatMap(f => {
  // CI checkouts live under /home/runner/..., Docker under /app/, Windows anywhere:
  // normalize to a repo-relative "tests/..." name before comparing with the baseline.
  const m = String(f.name).replace(/\\/g, "/").match(/tests\/.*$/);
  const rel = m ? m[0] : String(f.name);
