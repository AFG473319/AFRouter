...
# → {"request_id":"abc123"}   (response header x-afrouter-connection-id: <id>)
```
...
- Jobs are **account-bound** upstream: poll with the same connection that created the job (`x-connection-id` header, value from the create response's `x-afrouter-connection-id`).
