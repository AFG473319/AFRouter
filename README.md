<div align="center">
  <img src="./images/afrouter.png?1" alt="AFRouter Dashboard" width="800"/>
...
> **AFRouter** is a maintained personal fork of [9Router](https://github.com/decolua/9router) with extra providers and fixes not yet upstream: Nous Portal (Hermes CLI OAuth), OrcaRouter, concurrent model testing, and a multi-model Grok Build integration. Runs on port **20128** with its own data directory (`%APPDATA%/afrouter` / `~/.afrouter`), so it coexists with a stock 9Router install. Install: `npm install -g @afg473319/afrouter`.
  ...
  **Connect All AI Code Tools (Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) to 40+ AI Providers & 100+ Models.**
  ...
  [![npm](https://img.shields.io/npm/v/@afg473319/afrouter.svg)](https://www.npmjs.com/package/@afg473319/afrouter)
  [![Downloads](https://img.shields.io/npm/dm/@afg473319/afrouter.svg)](https://www.npmjs.com/package/@afg473319/afrouter)
...
This repository package is private (`afrouter-app`), so source/Docker execution is the expected local development path.
...
<table>
  ...
  <tr>
    ...
    <td align="center" width="320">
      ...
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Hướng Dẫn Setup OpenClaw + AFRouter: Tạo Bot Zalo AI Tự Động Từ A-Z<br/>by <a href="https://github.com/tuanminhhole">tuanminhhole</a></sub>
    </td>
...
> 🎬 **Made a video about AFRouter?** Submit a [Pull Request](https://github.com/AFG473319/AFRouter/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

AFRouter works seamlessly with all major AI coding tools:
...
Set `X-AFRouter-Token-Saver: off` to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. AFRouter calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:
...
If Headroom is down or returns an error, AFRouter fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → AFRouter translates → Provider receives native format
...
> AFRouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding AFRouter Costs & Billing

**AFRouter Billing Reality:**

✅ **AFRouter software = FREE forever** (open source, never charges)  
...
❌ **AFRouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, AFRouter just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **AFRouter**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through AFRouter.
...
<details>
<summary><b>💳 Will I be charged by AFRouter?</b></summary>

**No.** AFRouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, AFRouter just routes your requests
- ❌ **AFRouter itself** → **Never charges anything, ever**

AFRouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
AFRouter just routes your requests to them - there's no "catch" or future billing from AFRouter itself. They're truly free services, and AFRouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - AFRouter helps maximize their value through quota tracking
...
AFRouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and AFRouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. AFRouter tracks quota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from afrouter dashboard]
...
git pull && docker build -t afrouter .   # update to latest
```

**Data persistence:** `$HOME/.afrouter/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.

### Environment Variables

| Variable                                             | Default                                  | Description                                                                         |
| ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Auto-generated (`~/.afrouter/jwt-secret`) | JWT signing secret for dashboard auth cookie (override to share across instances)   |
| `INITIAL_PASSWORD`                                   | `123456`                                 | First login password when no saved hash exists                                      |
| `DATA_DIR`                                           | `~/.afrouter`                             | Main app data location (SQLite at `$DATA_DIR/db/data.sqlite`)                       |
...
- Both `${DATA_DIR}` and `~/.afrouter` resolve to the same location in a Docker container — the symlink `/root/.afrouter -> /app/data` is created at build time.
...
Thanks to all contributors who helped make AFRouter better!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of AFRouter. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. AFRouter ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. AFRouter adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. AFRouter injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, AFRouter's token-saving features wouldn't exist. ⭐ them on GitHub!
