...
**AFRouter menyelesaikan itu semua:**
...
Paket repo ini bersifat privat (`afrouter-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
### 📺 Panduan Setup Lengkap - AFRouter + Claude Code Gratis

[![AFRouter + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Tonton tutorial langkah demi langkah:**
- ✅ Install dan setup AFRouter
...
AFRouter bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → AFRouter mengonversi → provider menerima dalam format nativenya
...
> AFRouter sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing AFRouter!
...
### 📊 Tentang Biaya dan Penagihan AFRouter

**Fakta soal penagihan AFRouter:**

✅ **Software AFRouter = gratis selamanya** (open source, tanpa tagihan)
...
❌ **AFRouter tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, AFRouter hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **AFRouter**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat AFRouter.
...
<details>
<summary><b>💳 Apakah AFRouter menagih saya?</b></summary>

**Tidak.** AFRouter adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, AFRouter hanya me-routing request
- ❌ **AFRouter sendiri** → **tidak menagih apa pun**

AFRouter adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
AFRouter hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan AFRouter membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh AFRouter.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - AFRouter memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback AFRouter mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan AFRouter akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. AFRouter melacak kuota per model!
...
```bash
npm install -g afrouter
PORT=20128 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com afrouter
```
