<div align="center">
  <img src="../images/afrouter.png?1" alt="Tableau de bord AFRouter" width="800"/>
  
  # AFRouter - Routeur IA GRATUIT et économiseur de tokens
...
Le paquet de ce dépôt est privé (`afrouter-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
AFRouter fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. AFRouter appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → AFRouter → Headroom /v1/compress → AFRouter → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, AFRouter bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → AFRouter le traduit → Le fournisseur reçoit le format natif
...
> AFRouter lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de AFRouter

**La réalité de la facturation de AFRouter :**

✅ **Le logiciel AFRouter = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **AFRouter n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, AFRouter se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **AFRouter** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 AFRouter me facturera-t-il ?</b></summary>

**Non.** AFRouter est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, AFRouter route simplement vos requêtes
- ❌ **AFRouter lui-même** → **Ne facture jamais rien, jamais**

AFRouter est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
AFRouter se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et AFRouter les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - AFRouter vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de AFRouter évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et AFRouter les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. AFRouter suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord afrouter]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-afrouter"
}
```
...
export OPENAI_API_KEY="votre-clé-api-afrouter"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un AFRouter local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.afrouter/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
| `INITIAL_PASSWORD`                                   | `123456`                                    | Mot de passe de première connexion quand aucun hash n'est enregistré                          |
| `DATA_DIR`                                           | `~/.afrouter`                                | Emplacement principal des données de l'app (SQLite dans `$DATA_DIR/db/data.sqlite`)          |
...
- `${DATA_DIR}` et `~/.afrouter` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.afrouter -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par AFRouter
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de AFRouter. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. AFRouter porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. AFRouter adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. AFRouter injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de AFRouter n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
