...
const ENDPOINT = "/api/cli-tools/zcode-settings";

export default function ZCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const currentBaseUrl = status?.zcode?.baseURL || "";

  const getConfigStatus = () => {
    if (!status?.installed) return null;
    if (status.corrupt) return "corrupt";
    if (!status.hasAFRouter) return "not_configured";
    return matchKnownEndpoint(status.zcode?.baseURL || "", { tunnelPublicUrl, tailscaleUrl }) ? "configured" : "other";
  };
  ...
  // behave identically under later dashboard Reset runs.
  ...
  const unverifiedModels = status?.zcode?.unverified || [];
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src="/providers/zcode.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-lg" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} loading="lazy" decoding="async" />
          </div>
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-yellow-600 dark:text-yellow-400">ZCode not detected locally</p>
                    <p className="text-sm text-text-muted">Launch ZCode once so it creates ~/.zcode/v2/config.json, or configure it manually — Manual Config works for remote machines too.</p>
                  </div>
                </div>
          ...
          {!checking && status?.corrupt && (
            <div className="flex flex-col gap-3 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="font-medium text-red-600 dark:text-red-400">ZCode config is unreadable</p>
                  <p className="text-sm text-text-muted">{status.configPath} does not contain valid JSON. Restore the latest config.json.bak-* backup next to it, or fix the file by hand — AFRouter will not write to it until it parses.</p>
                </div>
              </div>
            </div>
          )}
      ...
      <ManualConfigModal
        ...
        title="ZCode - Manual Configuration"
