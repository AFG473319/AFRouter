...
async function buildCodexHeader() {
  ...
  const { installed, hasAFRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  // Extract AFRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:AFRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract AFRouter provider config
  const provider = settings?.models?.providers?.["afrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("afrouter/") ? primary.replace("afrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasAFRouter, opencode } = result.data;
