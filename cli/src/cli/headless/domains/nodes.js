...
async function run(action, pos, opts) {
  switch (action) {
    ...
    case "get": {
      ...
      const node = nodes.find(n => n.id === pos[0] || n.prefix === pos[0]);
      if (!node) return { error: `Node not found: ${pos[0]}` };
      return { data: node };
    }
    case "add": {
      const body = {
      ...
      if (opts["api-type"] || opts.apiType) body.apiType = opts["api-type"] || opts.apiType;
    ...
    case "edit": {
      ...
      const r = await api.updateProviderNode(pos[0], updates);
      if (!r.success) return { error: r.error };
      return { data: r.data };
    }
    case "delete": {
      const r = await api.deleteProviderNode(pos[0]);
      if (!r.success) return { error: r.error };
      return { data: r.data };
    }
    case "validate": {
