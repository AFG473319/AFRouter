...
// Subcommands (`afrouter xai video …`) run against an already-running gateway
...
// Identifiers for killAllAppProcesses - only kill afrouter specifically
const PROCESS_IDENTIFIERS = [
  'afrouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "afrouter")
    : path.join(os.homedir(), ".afrouter");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running afrouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "afrouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("afrouter") && (cmd.includes("cli.js") || cmd.includes("\\afrouter") || cmd.includes("/afrouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running afrouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "afrouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("afrouter") && (cmd.includes("cli.js") || cmd.includes("/afrouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 AFRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 AFRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "afrouter", "db.json") : path.join(".afrouter", "db.json"));
